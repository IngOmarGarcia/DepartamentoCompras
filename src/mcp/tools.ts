...
import {
  CrearPedidoInput, ConsultarStockInput, MovimientoInput, TransferenciaInput,
  CrearOrdenCompraInput, RecibirOrdenCompraInput, CrearProductoInput, ListarInput,
  SurtirPedidoInput, AprobarRequisicionInput, CancelarPedidoInput,
  CrearCategoriaInput, CrearUnidadInput, EstablecerStockInput,
} from '../schemas/index.js';

/** Definición declarativa de una herramienta MCP. */
export interface HerramientaMCP<S extends z.ZodTypeAny = z.ZodTypeAny> {
  nombre: string;
  titulo: string;
  descripcion: string;
  esquema: S;
  soloLectura: boolean;
  ejecutar: (ctx: Contexto, args: z.infer<S>) => Promise<unknown>;
}

function herramienta<S extends z.ZodTypeAny>(h: HerramientaMCP<S>): HerramientaMCP<z.ZodTypeAny> {
  return h as unknown as HerramientaMCP<z.ZodTypeAny>;
}
 ...
 */
export const HERRAMIENTAS: HerramientaMCP<z.ZodTypeAny>[] = [
  // ── INVENTARIO ────────────────────────────────────────────────────────────
  herramienta({
    nombre: 'inventario_consultar_stock',
    titulo: 'Consultar stock',
    descripcion:
      'Devuelve existencia total, cantidad reservada y disponible real por producto y por almacén. ' +
  ...
  herramienta({
    ...
    descripcion:
      'Registra entrada, salida, merma o ajuste. Nunca consume stock ya reservado por un pedido; ' +
  ...
  herramienta({
    ...
    descripcion:
      'Deja la existencia de un producto en un almacén EXACTAMENTE en la cantidad indicada. ' +
  ...
  herramienta({
    ...
    descripcion:
      'FLUJO PRINCIPAL. Da de alta el requerimiento y en la misma transacción valida stock: ' +
  ...
  herramienta({
    ...
    descripcion:
      'Genera entradas de inventario, recalcula costo promedio ponderado, cierra la requisición y ' +
  ...
  herramienta({
    ...
    descripcion:
      'Modifica campos del producto: mínimos, punto de reorden, lead time, categoría, costos o `activo`. ' +
  ...
  herramienta({
    ...
    descripcion:
      'Da de alta una categoría del catálogo. Con `padre_id` cuelga del árbol existente. ' +
  ...
  herramienta({
    ...
    descripcion:
      'Cambia una regla: permitir_stock_negativo, auto_generar_requisicion, reserva_parcial, ' +
