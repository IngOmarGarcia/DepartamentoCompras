...
import type { CrearOrdenCompraInput, RecibirOrdenCompraInput, ListarInput } from '../schemas/index.js';
import type { z } from 'zod';
import type { AprobarRequisicionInput } from '../schemas/index.js';

export const comprasService = {
  /** Bandeja de requisiciones — origen: faltante de stock o punto de reorden. */
  async listarRequisiciones(ctx: Contexto, filtros: ListarInput) {
    return consultar(async (client) => {
      let q = client
        .from('requisiciones')
        .select(
          `id, folio, origen, estatus, prioridad, fecha_requerida, pedido_id, creado_en,
      ...
      if (filtros.estatus) q = q.in('estatus', filtros.estatus.split(','));
      else q = q.not('estatus', 'in', '("cerrada","cancelada","rechazada")');
      if (filtros.buscar) q = q.ilike('folio', `%${filtros.buscar}%`);
      return q;
    });
  },

  async obtenerRequisicion(ctx: Contexto, id: string) {
  ...
  async aprobarRequisicion(ctx: Contexto, input: z.infer<typeof AprobarRequisicionInput>) {
  ...
  async sugerirProveedores(ctx: Contexto, requisicionId: string) {
    ...
    const productoIds = (items ?? []).map((i) => i.producto_id as string);
  ...
  async crearOrdenCompra(ctx: Contexto, input: CrearOrdenCompraInput) {
  ...
  async obtenerOrden(ctx: Contexto, id: string) {
    const { data, error } = await db
      .from('ordenes_compra')
      .select(
        `*, proveedor:proveedores(id, codigo, razon_social, dias_credito, contacto),
  ...
  async cambiarEstatusOrden(ctx: Contexto, id: string, estatus: 'enviada' | 'confirmada' | 'cancelada') {
    const { data, error } = await db
      .from('ordenes_compra')
      .update({ estatus })
      .eq('organizacion_id', ctx.organizacionId)
   ...
   */
  async recibirOrdenCompra(ctx: Contexto, input: RecibirOrdenCompraInput) {
  ...
  async listarOrdenes(ctx: Contexto, filtros: ListarInput) {
    return consultar(async (client) => {
      let q = client
        .from('ordenes_compra')
        .select(
          `id, folio, estatus, moneda, subtotal, impuestos, total, fecha_emision, fecha_promesa, creado_en,
  ...
  async listarProveedores(ctx: Contexto, filtros: ListarInput) {
    return consultar(async (client) => {
      let q = client
        .from('proveedores')
        .select('*', { count: 'exact' })
        ...
        .order('razon_social')
        .range(filtros.offset, filtros.offset + filtros.limite - 1);
      if (filtros.buscar) q = q.or(`razon_social.ilike.%${filtros.buscar}%,codigo.ilike.%${filtros.buscar}%`);
      return q;
    });
  },
};
