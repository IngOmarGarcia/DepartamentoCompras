...
import type { Contexto, ResultadoValidacionStock } from '../types/domain.js';
import type { CrearPedidoInput, ListarInput } from '../schemas/index.js';
import { z } from 'zod';
import { SurtirPedidoInput, CancelarPedidoInput } from '../schemas/index.js';
...
export const pedidosService = {
  ...
  async recibir(ctx: Contexto, input: CrearPedidoInput): Promise<ResultadoValidacionStock> {
    const payload = {
    ...
    const data = await rpc<ResultadoValidacionStock>(fn, { p_payload: payload });
    return normalizar(data);
  },

  /** Re-ejecuta la validación de stock (p. ej. tras una recepción de compra). */
  async validar(ctx: Contexto, pedidoId: string, usuarioId?: string): Promise<ResultadoValidacionStock> {
    const data = await rpc<ResultadoValidacionStock>('fn_procesar_pedido', {
  ...
  async surtir(ctx: Contexto, input: z.infer<typeof SurtirPedidoInput>) {
  ...
  async cancelar(_ctx: Contexto, input: z.infer<typeof CancelarPedidoInput>) {
  ...
  async listar(ctx: Contexto, filtros: ListarInput) {
    return consultar(async (client) => {
      let q = client
        .from('pedidos')
        .select(
          `id, folio, origen, referencia_externa, estatus, prioridad, fecha_requerida,
      ...
      if (filtros.buscar) q = q.or(`folio.ilike.%${filtros.buscar}%,referencia_externa.ilike.%${filtros.buscar}%`);
      return q;
    });
  },

  async obtener(ctx: Contexto, pedidoId: string) {
  ...
  async colaSurtido(ctx: Contexto) {
    return consultar(async (client) =>
      client
...
function normalizar(d: ResultadoValidacionStock): ResultadoValidacionStock {
  return {
    ...d,
