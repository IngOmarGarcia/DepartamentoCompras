...
async function main() {
    ...
    const ctx = await contextoDesdeApiKey(env.MCP_API_KEY);
    const server = new McpServer({ name: env.MCP_SERVER_NAME, version: '1.0.0' }, { capabilities: { tools: {}, logging: {} } });
    for (const h of HERRAMIENTAS) {
        const shape = h.esquema instanceof z.ZodObject ? h.esquema.shape : {};
        server.registerTool(h.nombre, {
            title: h.titulo,
            ...
            try {
                ...
                const err = e instanceof AppError
                    ...
                    : e instanceof z.ZodError
                        ...
                        : { ok: false, error: { codigo: 'ERROR_INTERNO', mensaje: e.message } };
    ...
    process.stderr.write(`[MCP] ${env.MCP_SERVER_NAME} listo · org=${ctx.organizacionId} · rol=${ctx.rol} · ${HERRAMIENTAS.length} herramientas\n`);
}
main().catch((e) => {
    process.stderr.write(`[MCP] Error fatal: ${e.message}\n`);
    process.exit(1);
});
