...
const redondear4 = (v) => Math.round(v * 1e4) / 1e4;
/** Resuelve SKUs → UUIDs dentro de la organización del contexto. */
async function resolverProductos(ctx, skus) {
    if (!skus.length)
        return [];
    ...
    const encontrados = new Set((data ?? []).map((r) => r.sku));
...
export const inventarioService = {
    ...
    async registrarMovimiento(ctx, input) {
        const data = await rpc('fn_registrar_movimiento', {
            p_org: ctx.organizacionId,
     ...
     */
    async establecerStock(ctx, input) {
        const { data, error } = await db
            .from('existencias')
    ...
    async kardex(ctx, filtros) {
        return consultar(async (client) => {
            ...
            if (filtros.producto_id)
                q = q.eq('producto_id', filtros.producto_id);
            if (filtros.almacen_id)
                q = q.eq('almacen_id', filtros.almacen_id);
            if (filtros.desde)
