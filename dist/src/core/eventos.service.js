...
export const eventosService = {
    ...
    async entregar(evento) {
        if (!env.WEBHOOK_URL)
            return true; // sin destino configurado: solo se marca como drenado
    ...
    async drenar(limite = 100) {
        ...
        for (const evento of cola) {
            if (await this.entregar(evento))
                entregados.push(evento.id);
            else
                fallidos += 1;
        }
