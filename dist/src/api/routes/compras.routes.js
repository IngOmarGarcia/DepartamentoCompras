...
import { AprobarRequisicionInput, CrearOrdenCompraInput, RecibirOrdenCompraInput, ListarInput, } from '../../schemas/index.js';
export async function comprasRoutes(app) {
    ...
    app.get('/requisiciones/:id', async (req) => ok(await comprasService.obtenerRequisicion(req.ctx, req.params.id)));
    /** GET /api/compras/requisiciones/:id/sugerencias — mejor proveedor por línea. */
    app.get('/requisiciones/:id/sugerencias', async (req) => ok(await comprasService.sugerirProveedores(req.ctx, req.params.id)));
    app.post('/requisiciones/:id/aprobar', async (req) => {
    ...
    app.get('/ordenes/:id', async (req) => ok(await comprasService.obtenerOrden(req.ctx, req.params.id)));
    app.patch('/ordenes/:id/estatus', async (req) => ok(await comprasService.cambiarEstatusOrden(req.ctx, req.params.id, req.body.estatus)));
    /**
     ...
     */
    app.post('/ordenes/:id/recepcion', async (req) => {
        const input = parse(RecibirOrdenCompraInput, { orden_compra_id: req.params.id, ...req.body });
