...
import { CrearProductoInput, CrearCategoriaInput, CrearUnidadInput, ListarInput, } from '../../schemas/index.js';
export async function catalogosRoutes(app) {
     ...
     */
    app.post('/productos', { preHandler: requiereRol('almacen') }, async (req) => ok(await catalogosService.crearProducto(req.ctx, parse(CrearProductoInput, req.body))));
    app.patch('/productos/:id', { preHandler: requiereRol('almacen') }, async (req) => ok(await catalogosService.actualizarProducto(req.ctx, req.params.id, parse(CrearProductoInput.partial(), req.body))));
    ...
    app.post('/categorias', { preHandler: requiereRol('almacen') }, async (req) => ok(await catalogosService.crearCategoria(req.ctx, parse(CrearCategoriaInput, req.body))));
    app.get('/unidades', async (req) => ok(await catalogosService.listarUnidades(req.ctx)));
    app.post('/unidades', { preHandler: requiereRol('almacen') }, async (req) => ok(await catalogosService.crearUnidad(req.ctx, parse(CrearUnidadInput, req.body))));
    ...
    app.put('/reglas/:clave', { preHandler: requiereRol('admin') }, async (req) => ok(await catalogosService.actualizarRegla(req.ctx, req.params.clave, req.body.valor, req.body.descripcion)));
}
export async function dashboardRoutes(app) {
    ...
    app.get('/auditoria', { preHandler: requiereRol('admin') }, async (req) => ok(await dashboardService.auditoria(req.ctx)));
}
