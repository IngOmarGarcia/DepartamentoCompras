...
import { CrearPedidoInput, ProcesarPedidoInput, SurtirPedidoInput, CancelarPedidoInput, ListarInput, } from '../../schemas/index.js';
export async function pedidosRoutes(app) {
    ...
    app.post('/:id/validar', async (req) => {
        const input = parse(ProcesarPedidoInput, { pedido_id: req.params.id, ...req.body });
    ...
    app.post('/:id/surtir', { preHandler: requiereRol('almacen') }, async (req) => {
        const input = parse(SurtirPedidoInput, { pedido_id: req.params.id, ...req.body });
        return ok(await pedidosService.surtir(req.ctx, input));
    });
    app.post('/:id/cancelar', async (req) => {
        const input = parse(CancelarPedidoInput, { pedido_id: req.params.id, ...req.body });
    ...
    app.get('/:id', async (req) => ok(await pedidosService.obtener(req.ctx, req.params.id)));
    ...
    app.post('/reservas/liberar-expiradas', { preHandler: requiereRol('almacen') }, async () => ok(await pedidosService.liberarExpiradas()));
}
