...
import { ConsultarStockInput, MovimientoInput, TransferenciaInput, EstablecerStockInput, ListarInput, } from '../../schemas/index.js';
export async function inventarioRoutes(app) {
    ...
    app.post('/movimientos', { preHandler: requiereRol('almacen') }, async (req) => ok(await inventarioService.registrarMovimiento(req.ctx, parse(MovimientoInput, req.body))));
    /** POST /api/inventario/existencias — fija la existencia a un valor absoluto (conteo/carga inicial). */
    app.post('/existencias', { preHandler: requiereRol('almacen') }, async (req) => ok(await inventarioService.establecerStock(req.ctx, parse(EstablecerStockInput, req.body))));
    app.post('/transferencias', { preHandler: requiereRol('almacen') }, async (req) => ok(await inventarioService.transferir(req.ctx, parse(TransferenciaInput, req.body))));
    ...
    app.post('/reabastecer', { preHandler: requiereRol('almacen', 'compras') }, async (req) => ok(await inventarioService.generarReabastecimiento(req.ctx)));
}
